...
class HuffmanCoder{
    ...
    stringify(node){  // converting the tree
    ...
    destringify(data){   //function to get tree from string during decoding
        let node = [];
        if(data[this.ind]==='\''){
            this.ind++;
    ...
    encode(data){
        ...
        this.mappings = {};  // to store mappings of characters
        this.getMappings(huffman_encoder, ""); // getting mappings

        let binary_string = ""; // converting whole text to binary string
        for(let i=0;i<data.length;i++) {
            binary_string = binary_string + this.mappings[data[i]];
        }

        let rem = (8 - binary_string.length%8)%8;  //padding the string if length is not multiple of 8
        let padding = "";
        for(let i=0;i<rem;i++)
            padding = padding + "0";
        ...
        for(let i=0;i<binary_string.length;i+=8){
            let num = 0;
            for(let j=0;j<8;j++){
                num = num*2 + (binary_string[i+j]-"0");  //converting to ascii value
            }
            result = result + String.fromCharCode(num); // getting ascci code
        }
    ...
    decode(data){
        ...
        for(let i=0;i<text.length;i++){
            ...
            let bin = "";
            for(let j=0;j<8;j++){
                bin = num%2 + bin;    // getting binary string from ascii value
        ...
        let res = "";
        let node = huffman_decoder;   //using huffmman decoer to get actual text back from huffmann decoder
